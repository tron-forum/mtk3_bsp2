...
このデバイスドライバは内部でFSP(HAL)を呼び出しますので、使用する際にはまず対象デバイスのFSPの設定をおこなってください。FSP(HAL)とデバイスドライバの紐づけは初期化関数 `dev_init_hal_adc()` にて行います。詳しくは次項で説明します。  
